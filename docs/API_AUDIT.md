...
No security finding is deferred or accepted. Live semantic qualification passed 60/60 for `gemini-3.5-flash-lite` (median 1.405 s, max 2.226 s) and 60/60 for Groq `openai/gpt-oss-120b` (median 1.855 s, max 4.812 s), using paced calls to avoid turning qualification into a quota stress test. All ten official public samples passed through the local service, the Dockerized real-provider API, and the public deployment at `https://gridlock-d.onrender.com`, including directive comparison, optimization, serialized replay, aggregate verification, and reference-cost comparison.
